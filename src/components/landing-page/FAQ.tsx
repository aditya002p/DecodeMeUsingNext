          ...
          {faqData.map((item, index) => (
            ...
            >
              <button
                ...
                className="w-full flex justify-between items-center px-3 sm:px-6 py-5 focus:outline-none"
              >
                <span className="font-medium text-base md:text-lg text-left">
                  {item.title}
                </span>
                {openItem === item.title ? (
                  <FiChevronUp className="h-5 w-5 flex-shrink-0 text-gray-700 ml-2" />
                ) : (
                  <FiChevronDown className="h-5 w-5 flex-shrink-0 text-gray-700 ml-2" />
                )}
              </button>
              
              <div 
                className={`px-3 sm:px-6 pb-5 text-gray-600 text-sm md:text-base transition-all duration-300 ${
